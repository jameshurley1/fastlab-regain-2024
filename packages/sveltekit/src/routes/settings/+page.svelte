<script lang="ts">
	...
	// Derive which area names the clinician has assigned to this patient.
	const assignedAreaNames = $derived(
		new Set((user.current?.groups ?? []).map((g: { area: string }) => g.area))
	);

	let patientAreas: string[] = $state(user.current?.patientAreas ?? []);

	function toggleArea(areaName: string) {
		if (assignedAreaNames.has(areaName)) return;
		if (patientAreas.includes(areaName)) {
			patientAreas = patientAreas.filter((a) => a !== areaName);
		} else {
			patientAreas = [...patientAreas, areaName];
		}
	}
				...
				<CardContent>
					...
					<div class="selector">
						<div class="human-body">
							...
							>
								<path
									class={assignedAreaNames.has('Head') || patientAreas.includes('Head') ? 'active' : ''}
							...
							>
								<path
									class={assignedAreaNames.has('Shoulders') || patientAreas.includes('Shoulders') ? 'active' : ''}
							...
							>
								<path
									class={assignedAreaNames.has('Arms') || patientAreas.includes('Arms') ? 'active' : ''}
							...
							>
								<path
									class={assignedAreaNames.has('Chest') || patientAreas.includes('Chest') ? 'active' : ''}
							...
							<svg
								...
								onclick={() => toggleArea('Stomach')}
								style={assignedAreaNames.has('Stomach') ? '' : 'cursor: pointer'}
							>
								<path
									class={assignedAreaNames.has('Stomach') || patientAreas.includes('Stomach') ? 'active' : ''}
							...
							>
								<path
									class={assignedAreaNames.has('Legs') || patientAreas.includes('Legs') ? 'active' : ''}
							...
							>
								<path
									class={assignedAreaNames.has('Hands') || patientAreas.includes('Hands') ? 'active' : ''}
					...
					<div class="legend">
						<h4>Orange areas are part of your programme. You can also tap additional areas to explore more exercises.</h4>
						<Button variant="raised" onclick={() => goto('/exercises')}>View my exercises</Button>
					</div>
				</CardContent>
			</Card>
		</Cell>
		<Cell span={8}>
			<Card>
				<CardContent>
					<h4>Settings</h4>
					<p>
						The body areas shown on the figure are the treatment areas your clinician has selected
						for your rehabilitation programme. Orange areas are assigned; blue areas are not
						currently included in your plan.
					</p>
					<p>
						If you think your assigned areas need to change, please speak to your clinician who can
						update your programme in the admin panel.
					</p>
					<h4>Legend</h4>
					<p>
						<span class="off">BLUE</span> — not assigned &nbsp;
						<span class="on">ORANGE</span> — assigned by your clinician
					</p>
					<h4>Notifications</h4>
					<div>
						<FormField>
